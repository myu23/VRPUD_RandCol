...
public class ColumnGeneration_RCXenumeration {
	public Data data;
	...
	public ColumnGeneration_RCXenumeration(Data d){
		this.data = d;
		this.nNode = d.nNode;
	...
	public double getUpperbound(){
		...
		String csvFile = "Data\\VRPUDX\\RandCol-multi\\"+data.fileName+"-q"+data.capacity+"-ii.csv";
	...
	public double solve()throws GRBException, InterruptedException, NumberFormatException, IOException {
		...
		try {
			...
			String logName = "log\\LP\\RC\\"+data.fileName+"-q"+data.capacity;
			...
			while(oncemore){
				...
				if (master.get(GRB.IntAttr.SolCount) > 0) {
					for (int p = 0; p < routes.size(); p++) {
						if (lambda.getElement(p).get(DoubleAttr.X) > 1E-5) {
								routes.get(p).lastActive = nIter;
						}
					}
				}
...
//				}
				...
				int stepSize = 1;
				DataHandler d = new DataHandler(data, numThreads, stepSize);
				d.readX(numNodes, data.capacity);
...
////////////////////////////////////////////////END OF BOUNDING PROCEDURE //////////////////////////////////////////////////////////////////////////
			...
			lptime = System.currentTimeMillis() - timer;
			...
			String filename = "GeneratedRoutes\\random_color\\"+data.fileName+"-q"+data.capacity+"-e";
			ArrayList<ArrayList<Route>> temp = new ArrayList<>();
			temp.add(routes);
			Functions.saveRoutes(filename, temp);
	...
	public double solveMIP()throws InterruptedException, NumberFormatException, IOException {
		int i, j, k;
		try {
			String logName = "log\\MIP\\RC\\"+data.fileName+"-q"+data.capacity;
			File f = new File(logName);
			f.delete();
			GRBEnv   env   = new GRBEnv(logName);
			GRBModel master = new GRBModel(env);
...
//			}
			...
			timer = System.currentTimeMillis();
			master.optimize();
			iptime = System.currentTimeMillis() - timer;
			// print master status
			double objval = Double.MAX_VALUE;
			...
			System.out.println("Upper bound found: "+upperbound);
            ArrayList<Route> solutionRoute = new ArrayList<>();

            if (master.get(GRB.IntAttr.SolCount) > 0) {
                    for (int p = 0; p < routes.size(); p++) {
                        if (lambda.getElement(p).get(DoubleAttr.X) > 0.5) {
                            solutionRoute.add(routes.get(p));
                            System.out.println(routes.get(p).route.toString() + " "+routes.get(p).cost);
                        }
                    }

            }
			ArrayList<ArrayList<Route>> container = new ArrayList<>();
            container.add(solutionRoute);
			String filename = "IPSolutionRoute\\random_color\\"+data.fileName+"-q"+data.capacity;
			Functions.saveRoutes(filename, container);
	...
	public void loadRoutes(){
		String filename = "GeneratedRoutes\\random_color\\"+data.fileName+"-q"+data.capacity;
		ArrayList<ArrayList<Route>> temp = new ArrayList<>();
		temp.add(routes);
		Functions.saveRoutes(filename, temp);
	}
