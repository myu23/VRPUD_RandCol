...
public class ColumnGeneration_PMX {
	public Data data;
	...
	public ColumnGeneration_PMX(Data d){
		this.data = d;
		this.nNode = d.nNode;
	...
	public double solve()throws InterruptedException, NumberFormatException, IOException {
		int i, j, k;
		try {
			String logName = "log\\LP\\P\\"+data.fileName+"-q"+data.capacity;
...
//			System.out.println(sum);
			...
			String filename = "GeneratedRoutes\\pulse\\"+data.fileName+"-q"+data.capacity;
	...
	public double solveMIP()throws InterruptedException, NumberFormatException, IOException {
		int i, j, k;
		try {
			String logName = "log\\MIP\\P\\"+data.fileName+"-q"+data.capacity;
			File f = new File(logName);
			f.delete();
			GRBEnv   env   = new GRBEnv(logName);
			GRBModel master = new GRBModel(env);
...
//			}
			...
			String filename = "IPSolutionRoute\\pulse\\"+data.fileName+"-q"+data.capacity;
