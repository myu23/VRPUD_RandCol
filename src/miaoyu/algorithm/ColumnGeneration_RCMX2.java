...
public class ColumnGeneration_RCMX2 {
	public Data data;
	...
	public ColumnGeneration_RCMX2(Data d){
		this.data = d;
		this.nNode = d.nNode;
...
//			}
			...
			GRBVarArray lambda = new GRBVarArray();
			for(int p = 0; p < routes.size(); p++){
				if(routes.get(p).lastActive > nIter-50)
					lambda.add(master.addVar(0, 1, 0, GRB.BINARY, "lambda"+p));
				else
					lambda.add(master.addVar(0, 0, 0, GRB.BINARY, "lambda"+p));
			}
			...
			upperbound = master.get(DoubleAttr.ObjBound);
