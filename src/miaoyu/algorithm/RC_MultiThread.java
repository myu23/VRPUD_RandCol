...
public class RC_MultiThread {
    public Data data;
    public Set<Route> solutionSet;

    public RC_MultiThread(Data d){
        this.data = d;
...
//                            continue;
                            if ((i == data.nNode || (!current.nodeVisited[color[i]]) && (data.cost[current.endNode][i] < 1e6))) {  // don't go back to a vertex already visited or along a forbidden edge
                                // time
